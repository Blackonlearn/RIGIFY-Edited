...
class Generator(base_generate.BaseGenerator):
    ...
    def ensure_rig_object(self) -> ArmatureObject:
        ...
        if not target_rig:
            ...
            # Create a new armature and target_rig object
        ...
        if target_rig in list(self.context.scene.collection.all_objects):
            ...
            # Rename target_rig if its name is different from rig_new_name
            if rig_basename:  
                if target_rig.name != rig_new_name:
                    target_rig.name = "RIG-" + rig_new_name
                    target_rig.data.name = "RIG-" + rig_new_name
                if self.metarig.name != rig_new_name:
                    self.metarig.name = "META-" + rig_new_name
                    self.metarig.data.name = "META-" + rig_new_name
