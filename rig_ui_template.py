...
from .utils.rig import get_rigify_layers, get_rigify_rig_basename
...
class ScriptGenerator(base_generate.GeneratorPlugin):
    ...
    def __init__(self, generator):
        ...
        # Add a dynamic category for the N-panel
        rig_basename = get_rigify_rig_basename(generator.metarig.data)
        self.bl_category = 'RIG-' + rig_basename if rig_basename else "Rigify"
    ...
    def finalize(self):
        ...
        script.write(UI_SLIDERS.format(bl_category=self.bl_category))
        ...
        if self.use_bake_settings:
            ...
            script.write(UI_BAKE_SETTINGS.format(bl_category=self.bl_category))

        script.write(layers_ui(vis_layers, layer_layout).format(bl_category=self.bl_category))
